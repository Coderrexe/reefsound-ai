...
for f in audio_fs:
    ...
    # Extract timestamp from filename - this only works for our dataset due to the specific file name convention.
    t2 = f.split(".")[1][0:4]
    t1 = f.split(".")[3]
    t = t1 + t2 + "00"
    recording_start_time = pd.to_datetime(t, format="%y%m%d%H%M%S")  # e.g. Timestamp('2018-08-27 10:55:00')
